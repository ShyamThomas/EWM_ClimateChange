...
#### Figure 1: Maps showing EWM distribution and lake temperatures in GDD
Minn.sf
...
library("patchwork")
EWMmap|CurrGDDmap|FutrGDDmap
ggsave("Mnspt.Fig1.png", path="Figures/", device="png",width=12, height=4, dpi=900)

############################  
### Figure 2: Boxplots comparing predicted EWM invasion risk under current and future water temperature/GDD
...
PeriodWise
####################################################################################
### Figure 3: Arrow plots showing change in risk over time
EWM.clmchng_data ### from Line 6 above
...
### Figure 4: Final EWM invasion risk predictions and uncertainty maps, analog/non-analog domains
...
Fig.4a=ggplot(data=Minn.sf)+geom_sf()+
          geom_sf(data=IncreasersUncertain_Lakes_sf_WGS_recoded, aes(color=as.factor(Status)))+theme_light()+
...
EWM.futrpreds.bayes.GAM_k10=EWM.clmchng_data%>%select(1,3,4,58,59)%>%rename(Prediction = bestmodel.bayesian.meanFutrPreds, Variance = bestmodel.bayesian.varFutrPreds)
...
EWM.futrpreds.bayes.GAM_k10_sf_WGS=st_transform(EWM.futrpreds.bayes.GAM_k10, crs=4326)
...
Fig.4b=ggplot(data=Minn.sf)+geom_sf()+
          geom_sf(data=EWM.futrpreds.bayes.GAM_k10_sf_WGS, aes(color=Prediction, size=Variance))+theme_light()+
          scale_color_viridis_c(option="turbo",alpha = 0.75)+
...
Fig.4c=ggplot(data=Minn.sf)+geom_sf()+
          geom_sf(data=EWM.domains.sf.WGS, aes(color=Domain))+theme_light()+
          scale_color_viridis_d(option="turbo",alpha = 0.75)+theme(legend.title = element_blank())+
          theme(legend.position = c(0.8,0.45))+theme(text=element_text(size=16))
Fig.4c
